...
// test all variations.
func makeProvers(trie *Trie) []func(key []byte) *dbm.GoMemDB {
	var provers []func(key []byte) *dbm.GoMemDB

	// Create a direct trie based Merkle prover
	provers = append(provers, func(key []byte) *dbm.GoMemDB {
	...
	for i, prover := range makeProvers(trie) {
		for _, kv := range vals {
			...
			if !bytes.Equal(val, kv.v) {
				t.Fatalf("prover %d: verified value mismatch for key %x: have %v, want %x", i, kv.k, val, kv.v)
			}
		}
	}
}
...
func BenchmarkProve(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		proofs, _ := dbm.NewGoMemDB("gomemdb", "", 128)
