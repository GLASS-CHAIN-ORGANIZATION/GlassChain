	...
	for i, test := range packUnpackTests {
		t.Run(strconv.Itoa(i), func(t *testing.T) {
			...
			if !reflect.DeepEqual(packed[4:], encb) {
				...
				t.Errorf("expected: %v", encb)
				t.Errorf("got     : %v", packed[4:])
			}
		})
	}
}
...
func TestMethodPack(t *testing.T) {
	...
	packed, err = abi.Pack("sliceAddress", []common.Hash160Address{addrA.ToHash160(), addrB.ToHash160()})
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(packed, sig) {
	...
	packed, err = abi.Pack("sliceMultiAddress", []common.Hash160Address{addrA.ToHash160(), addrB.ToHash160()}, []common.Hash160Address{addrC.ToHash160(), addrD.ToHash160()})
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(packed, sig) {
	...
	sig = append(sig, common.LeftPadBytes(addrC.Bytes(), 32)...)
	sig = append(sig, common.LeftPadBytes(addrD.Bytes(), 32)...)
	packed, err = abi.Pack("nestedArray", a, []common.Hash160Address{addrC.ToHash160(), addrD.ToHash160()})
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(packed, sig) {
